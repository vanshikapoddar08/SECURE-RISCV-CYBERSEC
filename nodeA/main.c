...
static void handle_incoming_data(Packet *rx, Packet *tx, uint32_t peer_uart,
                                 uint8_t my_id, uint32_t python_uart)
{
                                ...
                                uint32_t peer_uart)
{
    ...
    if (reciving_from_python(buf, UART0_BASE) == (char)-1) {
        //uart_puts(UART0_BASE, "[A] buffer overflow, input dropped\n");
        return 0;
    }
...
int main(void)
{
    ...
    while (1)
    {
        ...
        case STATE_DISCONNECTED:
           // uart_puts(UART0_BASE, "[A] sending SYN\n");
            send_ctrl(&tx_packet, UART1_BASE, PKT_SYN);
            state = STATE_WAITING_SYN_ACK;
            break;

        case STATE_WAITING_SYN_ACK:
            if (packet_receive_with_timeout(&rx_packet, UART1_BASE,
                                            HANDSHAKE_TIMEOUT_MS, my_id) == PKT_RECV_OK)
            {
                if (handle_unexpected_syn(&rx_packet, &tx_packet, UART1_BASE))
                    break;

                if (rx_packet.type == PKT_SYN_ACK)
                {
                    send_ctrl(&tx_packet, UART1_BASE, PKT_ACK);
                    uart_puts(UART0_BASE, "[A] connected to Node B\n");
            ...
            {
                uart_puts(UART0_BASE, "[A] SYN timeout, retrying\n");
        ...
        case STATE_CONNECTED:
            ...
            if (packet_receive_with_timeout(&rx_packet, UART1_BASE,
                                            CONNECTED_POLL_MS, my_id) == PKT_RECV_OK)
            {
                if (handle_unexpected_syn(&rx_packet, &tx_packet, UART1_BASE))
                {
                    state = STATE_WAITING_SYN_ACK;
        ...
        case STATE_WAITING_DATA_ACK:
            ...
            {
                cheker++;
                uart_puts(UART0_BASE, "[A] ACK timeout, retransmitting\n");
                if (cheker >= MAX_ACK_RETRIES)
                {
                    uart_puts(UART0_BASE, "[A] giving up, reconnecting\n");
                    state  = STATE_DISCONNECTED;
                    cheker = 0;
                }
                else
